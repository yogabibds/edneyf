...
(function(){
  ...
  let idx = 0;
  function openLB(i){ idx=(i+imgs.length)%imgs.length; const el=imgs[idx]; lbImg.src=el.src; lbCap.textContent=el.dataset.caption||el.alt||''; lb.classList.add('open'); document.body.style.overflow='hidden';}
  ...
  if(toolbar){ const btnSlide=toolbar.querySelector('[data-slideshow]'); const btnGrid=toolbar.querySelector('[data-grid]'); if(btnSlide) btnSlide.addEventListener('click', ()=> openLB(0)); if(btnGrid) btnGrid.addEventListener('click', ()=> closeLB()); }
})();